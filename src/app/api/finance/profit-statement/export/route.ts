...
// GET /api/finance/profit-statement/export?year=2026&anonymize=true|false
export async function GET(req: NextRequest) {
  ...
  const [result, journalRaw, branding] = await Promise.all([
    computeProfitStatement(userId, role, year),
    computeTransactionJournal(userId, role, year, anonymize),
    getBranding(),
  ])
  ...
  const journal = journalRaw.map(e => ({ ...e, date: fmtDate(e.date) }))
  ...
  await prisma.auditLog.create({
    data: { userId, action: 'FINANCE_DATA_EXPORTED', details: { action: 'profit_statement_pdf', year, anonymize } },
  }).catch(() => {})

  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="Einnahmen-Ausgaben-Aufstellung-${year}${anonymize ? '-anonym' : ''}.pdf"`,
    },
  })
}
