...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.duplicateFrom) {
      const src = await prisma.cooperationPartnerInvoiceTemplate.findUnique({ where: { id: body.duplicateFrom } })
      if (src) {
    ...
    if (data.isDefault) await prisma.cooperationPartnerInvoiceTemplate.updateMany({ data: { isDefault: false } })
    const template = await prisma.cooperationPartnerInvoiceTemplate.create({ data })
