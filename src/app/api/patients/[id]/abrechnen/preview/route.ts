...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const lineItems = sessions.flatMap(s => {
    ...
    const baseLine = removedKeys.has(baseKey) ? null : {
      ...
      serviceLabel: baseOverride?.serviceLabel !== undefined ? baseOverride.serviceLabel : (s.serviceLabel ?? null),
    ...
    const extraLines = (serviceLinesBySession.get(s.id) ?? [])
      ...
      .map(l => {
        ...
        return {
          ...
          serviceLabel: lineOverride?.serviceLabel !== undefined ? lineOverride.serviceLabel : (l.catalogCode ?? null),
          quantity: qty,
