...
// Leere Formularfelder ergeben parseFloat('')/parseInt('') = NaN — das an ein
// Prisma-Decimal/Int-Feld übergeben lässt den kompletten PATCH-Request mit 500
// abstürzen (auch wenn firstName/lastName etc. gültig gewesen wären). NaN → null.
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const patient = await prisma.patient.update({
    ...
    data: {
      ...
      ...(body.defaultUnitDuration  !== undefined && { defaultUnitDuration: numOrNull(parseInt(body.defaultUnitDuration)) }),
      ...(body.defaultUnitPriceNet  !== undefined && { defaultUnitPriceNet: numOrNull(parseFloat(body.defaultUnitPriceNet)) }),
      ...(body.defaultHourlyRateNet !== undefined && { defaultHourlyRateNet: numOrNull(parseFloat(body.defaultHourlyRateNet)) }),
      ...(body.sessionStartNumber   !== undefined && { sessionStartNumber: numOrNull(parseInt(body.sessionStartNumber)) ?? 0 }),
      ...(body.defaultVatRate       !== undefined && { defaultVatRate: numOrNull(parseFloat(body.defaultVatRate)) ?? 0 }),
