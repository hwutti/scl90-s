...
export function KooperationspartnerRechnungsvorlageClient({
  ...
  const refreshPreview = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/cooperation-partner-invoice-templates/preview', {
        method: 'POST',
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(`/api/cooperation-partner-invoice-templates/${activeId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function createNew() {
    setSaving(true)
    try {
      const res = await fetch('/api/cooperation-partner-invoice-templates', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: 'Neue Vorlage',
  ...
  async function setDefault() {
    if (!activeId) return
    try {
      await fetch(`/api/cooperation-partner-invoice-templates/${activeId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteTemplate() {
    ...
    await fetch(`/api/cooperation-partner-invoice-templates/${activeId}`, { method: 'DELETE' })
  ...
  function clearImage(field: 'headerImage' | 'footerImage' | 'bgImage' | 'signatureImage') {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>Kooperationspartner-Rechnungsvorlage</span>
