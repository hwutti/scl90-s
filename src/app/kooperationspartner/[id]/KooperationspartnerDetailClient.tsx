...
export function KooperationspartnerDetailClient({
  partner, invoiceTemplates, role, branding,
}: {
  partner: Partner
  ...
  function openEdit(tx: any) {
    ...
    setEditLineItems(tx.lineItems?.map((li: any) => ({
      id: li.id, description: li.description, descriptionHtml: li.descriptionHtml ?? '',
            ...
            {transactions.map((tx: any) => (
              <div key={tx.id} className="card" style={{ padding: 16 }}>
                ...
                {/* Bearbeitungs-Panel im echten Rechnungs-Look. Beträge sind bereits
                    ausgestellt und bleiben unveränderlich (§ Rechnungslegung) -- nur
                    Beschreibungstext und Freitext lassen sich noch anpassen. */}
                {editTxId === tx.id && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{
                      ...
                      <div style={{ paddingTop: 10, borderTop: '0.5px solid #eee' }}>
                        ...
                        <RichTextEditor value={editNoteHtml} onChange={setEditNoteHtml} placeholder="Optionaler Freitext…" minHeight={70} />
                      </div>
                    </div>
