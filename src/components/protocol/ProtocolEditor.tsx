
'use client'
import { useState, useEffect } from 'react'
import { Save, ChevronDown, ChevronUp, Lock, FileText } from 'lucide-react'

type ProtocolType = 'SHORT' | 'LONG'

const DEFAULT_SHORT_SECTIONS = [
  { title: 'Sitzungsinhalt', content: '' },
  { title: 'Interventionen', content: '' },
  { title: 'Nächste Schritte', content: '' },
]

const DEFAULT_LONG_SECTIONS = [
  { title: 'Subjektiver Eindruck', content: '' },
  { title: 'Übertragung / Gegenübertragung', content: '' },
  { title: 'Hypothesen', content: '' },
  { title: 'Therapeutische Beziehung', content: '' },
  { title: 'Persönliche Reflexion', content: '' },
]

export function ProtocolEditor({ sessionId, type, readOnly = false }: {
  sessionId: string; type: ProtocolType; readOnly?: boolean
}) {
  const [sections, setSections] = useState<{ title: string; content: string }[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)
  const [collapsed, setCollapsed] = useState(false)

  useEffect(() => {
    fetch(`/api/therapy-sessions/${sessionId}/protocol?type=${type}`)
      .then(r => r.json())
      .then(data => {
        if (data?.sections?.length) {
          setSections(data.sections)
        } else {
          setSections(type === 'SHORT' ? DEFAULT_SHORT_SECTIONS : DEFAULT_LONG_SECTIONS)
        }
        setLoading(false)
      })
      .catch(() => {
        setSections(type === 'SHORT' ? DEFAULT_SHORT_SECTIONS : DEFAULT_LONG_SECTIONS)
        setLoading(false)
      })
  }, [sessionId, type])

  async function save() {
    setSaving(true)
    await fetch(`/api/therapy-sessions/${sessionId}/protocol`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type, sections }),
    })
    setSaving(false)
    setSaved(true)
    setTimeout(() => setSaved(false), 2000)
  }

  const isLong = type === 'LONG'
  const label = isLong ? 'Langprotokoll' : 'Kurzprotokoll'
  const color = isLong ? 'var(--amber)' : 'var(--color-primary)'
  const bg = isLong ? 'var(--amber-bg)' : 'var(--color-primary-light)'

  return (
    <div style={{ border: `1px solid ${isLong ? 'var(--amber-border,#fcd34d)' : 'var(--border)'}`, borderRadius: 10, overflow: 'hidden', marginBottom: 12 }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: bg, cursor: 'pointer' }}
        onClick={() => setCollapsed(c => !c)}>
        <FileText style={{ width: 14, height: 14, stroke: color, fill: 'none', flexShrink: 0 }} />
        <span style={{ fontSize: 13, fontWeight: 600, color, flex: 1 }}>{label}</span>
        {isLong && <Lock style={{ width: 12, height: 12, stroke: color, fill: 'none' }} title="Nur für Therapeut*in sichtbar" />}
        {collapsed ? <ChevronDown style={{ width: 14, height: 14, stroke: color }} /> : <ChevronUp style={{ width: 14, height: 14, stroke: color }} />}
      </div>

      {!collapsed && (
        <div style={{ padding: 14, background: 'var(--surface-card)', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {loading ? (
            <div className="empty-state"><div className="spinner" style={{ width: 20, height: 20 }} /></div>
          ) : (
            <>
              {sections.map((sec, i) => (
                <div key={i}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 4 }}>
                    {sec.title}
                  </label>
                  <textarea
                    readOnly={readOnly}
                    value={sec.content}
                    onChange={e => {
                      const updated = [...sections]
                      updated[i] = { ...updated[i], content: e.target.value }
                      setSections(updated)
                    }}
                    placeholder={readOnly ? '' : `${sec.title} eingeben...`}
                    rows={3}
                    style={{
                      width: '100%', resize: 'vertical', padding: '8px 10px',
                      border: '0.5px solid var(--border-strong)', borderRadius: 7,
                      background: readOnly ? 'var(--surface-panel)' : 'var(--surface-input)',
                      color: 'var(--text-primary)', fontSize: 13, lineHeight: 1.6,
                      outline: 'none', fontFamily: 'inherit',
                      boxSizing: 'border-box',
                    }}
                    onFocus={e => !readOnly && (e.target.style.borderColor = color)}
                    onBlur={e => e.target.style.borderColor = 'var(--border-strong)'}
                  />
                </div>
              ))}
              {!readOnly && (
                <button onClick={save} disabled={saving} className="btn-primary" style={{ alignSelf: 'flex-end', fontSize: 12 }}>
                  <Save style={{ width: 12, height: 12 }} />
                  {saving ? 'Speichern...' : saved ? '✓ Gespeichert' : 'Protokoll speichern'}
                </button>
              )}
            </>
          )}
        </div>
      )}
    </div>
  )
}
